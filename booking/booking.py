...
class Booking(webdriver.Chrome):
    def __init__(self, driver_path = r'C:\Windows', teardown = False):
    ...
    def select_adults(self, count=1):
        ...
        while True:
            decrease_adult_element = self.find_element(
                By.CSS_SELECTOR, 'button[aria-label="Взрослых: уменьшить количество"]'
            )
        ...
        for _ in range(count-1):
            increase_button_element = self.find_element(
                By.CSS_SELECTOR, 'button[aria-label="Взрослых: увеличить количество"]'
            )
